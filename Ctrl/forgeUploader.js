 ...
 * @param bucketKey
 */
var getBucketDetails = function (bucketKey, oAuth2TwoLegged) {
 ...
 * @param bucketKey
 */
var createBucket = function (bucketKey, oAuth2TwoLegged) {
 ...
 * Uses the oAuth2TwoLegged object that you retrieved previously.
 */
var getBuckets = function(oAuth2TwoLegged){
 ...
 */
var createBucketIfNotExist = function (bucketKey, oAuth2TwoLegged) {

	return new Promise(function(resolve, reject) {
		getBucketDetails(bucketKey, oAuth2TwoLegged).then(function (resp) {
				resolve(resp);
			},
			function (err) {
				if (err.statusCode === 404) {
					createBucket(bucketKey, oAuth2TwoLegged).then(function(res){
							resolve(res);
						},
						function(err){
							reject(err);
...
var uploadFileCheck = function(bucketKey, filePath, fileName, oAuth2TwoLegged) {
     return new Promise(function (resolve, reject) {
        fs.readFile(filePath, function (err, data) {
            if (err) {
                ...
                if (data.length < 5242879){ // Less than 5MB files upload process
                    ...
                    resolve(uploadFile(bucketKey, filePath, fileName, oAuth2TwoLegged));
                }
                else{
                    ...
                    resolve(uploadFileChunk(bucketKey, filePath, fileName, oAuth2TwoLegged));
                }
            }
        })
    })
 ...
 * @returns {Promise}
 */
var uploadFile = function(bucketKey, filePath, fileName, oAuth2TwoLegged){
	return new Promise(function(resolve, reject) {
		fs.readFile(filePath, function (err, data) {
			if (err){
				reject(err);
 ...
 * @returns {Promise}
 */
var uploadFileChunk = function(bucketKey, filePath, fileName, oAuth2TwoLegged) {
     return new Promise(function (resolve, reject) {
        fs.readFile(filePath, function (err, data) {
            if (err) {
                reject(err);
 ...
 */
var translateFile = function(encodedURN, oAuth2TwoLegged){
...
var manifestFile = function (encodedURN, oAuth2TwoLegged) {
 ...
 */
var uploadFile = function(bucketkey, filename, filePath, oAuth2TwoLegged, callback){

	oAuth2TwoLegged.authenticate().then(function(credentials){

		createBucketIfNotExist(bucketkey, oAuth2TwoLegged).then(

			function(createBucketRes){

				getBuckets(oAuth2TwoLegged).then(function(getBucketsRes){
				...
				//console.log("uploadFileCheck")
	            uploadFileCheck(bucketkey, filePath, filename, oAuth2TwoLegged).then(function(uploadRes){
	            	
					const urnEncode = new Buffer(uploadRes.body.objectId).toString('base64');

					translateFile(urnEncode, oAuth2TwoLegged).then(function(translateRes){
						logs(chalk.bold.green("**** Translating file:") + chalk.bold.blue(urnEncode));

						manifestFile(urnEncode, oAuth2TwoLegged).then(function(){
